...
public class UpdationHandler {
	...
	public int update(Employee employeeData)
	{
	...
	public int update(Manager managerData)
	{
	...
	public int update(Customer customerData)
	{
	...
	public int update(Customer customerData, boolean status)
	{
		query="UPDATE Customer SET status = " + (status?1:0) +" where CID='"+customerData.getCID()+"'";
	...
	public boolean update(Account accountData)
	{
		try {
			...
			if (connection.executeUpdate(query)>0) 
				return true;
			return false;
		}
		catch (Exception ex) {
	...
	public int update(RegisteredCourier courierData)
	{
		query = "update regcourier set status = '" + courierData.getStatus() + "' where CID = '" + courierData.getCID() + "'";
		
		System.out.println(query);
		return connection.executeUpdate(query);
	}
	
	public int update(UnRegisteredCourier courierData)
	{
		query = "update unregcourier set status = '" + courierData.getStatus() + "' where CID = '" + courierData.getCID() + "'";
		System.out.println(query);
		return connection.executeUpdate(query);
	}	
	
	public int cancelCourier(String courierID) {
		if (courierID.charAt(0) == 'R'){
			query = "update regcourier set  status = 'Cancelled' where CID = '" + courierID + "'";
		}else {
			query = "update unregcourier set  status = 'Cancelled' where CID = '" + courierID + "'";
		}
